    ...
    async (currentOffset) => {
      ...
      try {
        ...
        if (!indexName) {
          ...
          const url = `${apiUrl}/api/trading/watchlist?limit=${limit}&offset=${currentOffset}`;
          console.log(`[useAllStocks] Fetching: ${url} (limit=${limit})`);
          ...
          const response = await fetch(
            `${apiUrl}/api/indices/${indexName}/stocks`
          );
          const result = await response.json();
